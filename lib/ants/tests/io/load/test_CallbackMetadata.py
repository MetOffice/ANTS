...
def test_existing_metadata_error(tmp_path):
    ...
    temporary_license_path.write_text(license_text, encoding="utf-8")
    error_message="The license is already an attribute on the cube. To ignore metadata files, use the --ignore-metadata-files flag."
...
def test_misspelt_license_warning(tmp_path):
    ...
    warning_message = "The attribute name lisense has been changed to license, in line with ANTS working practices."
...
def test_misspelt_license_added(tmp_path):
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore", message=warning_message, category=UserWarning
        )
...
def test_invalid_metadata_name(tmp_path):
    ...
    warning_message = "Attribute invalid-name is not a valid metadata file name. Accepted metadata names are license, attribution and restrictions."
    with pytest.raises(UserWarning, match=warning_message):
        ants.io.load.load_cube(temporary_cube_path)
